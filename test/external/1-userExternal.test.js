...
describe('User External', () => {
    describe('POST /users/register', () => {       
     it('Quando e-mail já existir na base de dados retornar erro', async() => {
            ...
            await request('http://localhost:3000').post('/api/users/register').send(dados);

            const resposta = await request('http://localhost:3000').post('/api/users/register').send(dados);
